...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>Generated by PhantomStrike v3.0.0-alpha | Authorized Testing Only</p>
...
class ReportEngine(BaseModule):
    ...
    async def run(self, target: str, options: dict = None) -> ModuleResult:
        ...
        txt_path = await self._generate_txt_report(target, scan_results, session_id)
    ...
    async def _generate_txt_report(self, target: str, results: dict, session_id: str) -> Path:
        ...
        lines = [
            "====================================================",
            "🔥 PHANTOM STRIKE - PENETRATION TEST REPORT",
            "====================================================",
            f"Target:    {target}",
            f"Date:      {timestamp}",
            ...
            "📊 EXECUTIVE SUMMARY",
            "-------------------",
            f"Critical:  {len([v for v in all_vulns if v.get('severity') == 'critical'])}",
            f"High:      {len([v for v in all_vulns if v.get('severity') == 'high'])}",
            f"Medium:    {len([v for v in all_vulns if v.get('severity') == 'medium'])}",
            f"Low:       {len([v for v in all_vulns if v.get('severity') in ('low', 'info')])}",
            "",
            "🚨 FINDINGS",
            "-----------",
        ]
        ...
        if not all_vulns:
            ...
            for i, vuln in enumerate(all_vulns, 1):
                lines.append(f"{i}. [{vuln.get('severity', 'medium').upper()}] {vuln.get('type', 'Unknown').upper()}")
                lines.append(f"   URL:      {vuln.get('url', 'N/A')}")
                lines.append(f"   Payload:  {vuln.get('payload', 'N/A')}")
                lines.append(f"   Mitre:    {MITRE_TECHNIQUE_MAP.get(vuln.get('type', ''), {}).get('technique', 'N/A')}")
                lines.append(f"   Fix:      {self._get_remediation(vuln.get('type', ''))}")
                lines.append("")

        lines.append("====================================================")
        lines.append("Generated by PhantomStrike v3.0.0-alpha")
        lines.append("====================================================")

        content = "\n".join(lines)
        timestamp_file = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = self._output_dir / f"phantom_report_{target.replace('.', '_')}_{timestamp_file}.txt"
